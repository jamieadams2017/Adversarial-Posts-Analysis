...
def get_gspread_sheet(sheet_id: str):
    """Cached gspread Spreadsheet handle (avoids re-auth on every rerun)."""
    creds = get_creds()
...
# ---------- render ----------
for name, tab in [("All", t_all), ("Facebook", t_fb), ("YouTube", t_yt), ("TikTok", t_tt)]:
    # NOTE: avoid keeping a global ALL_DF copy in memory (Render free tier is tight).
    # Build the "All" view on-demand.
    d0 = pd.concat([fb, yt, tt], ignore_index=True) if name == "All" else DATASETS[name]
